    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                ...
                if (row > 0) {
                   // ResultSet rs = st.executeQuery("Select * from campaign where id='"+campaign_id+"'");
                   // rs.next();
                   // int preValue =rs.getInt("AmountCol");
                   // int totalValve= preValue+Integer.parseInt(amount);
                   //  int i = st1.executeUpdate("update campaign set AmountCol='"+totalValve+"' where id='" + campaign_id + "'");
